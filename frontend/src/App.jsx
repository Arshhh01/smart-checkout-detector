...
export default function App() {
  ...
  function fetchAlerts() {
    setRefreshing(true);
    fetch(`${API_URL}/alerts/?limit=50`, { headers: { "X-API-Key": API_KEY } })
      .then((res) => res.json())
      .then((data) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const data = (parsed.type === "detection" || parsed.type === "alert") ? parsed.data : parsed;
      if (!data) return;
      ...
      if (data.scanned_items && data.scanned_items.length > 0) {
        setClearedItems((prev) => {
          ...
          for (const item of data.scanned_items) {
            if (!updated.find((c) => c.track_id === item.track_id)) {
              updated.unshift({
      ...
      if (data.is_alert && data.alert_reason) {
        ...
        setActiveThefts((prev) => {
          ...
          if (existing) {
            ...
            if ((sevOrder[newSeverity] || 0) > (sevOrder[existing.severity] || 0) || newDwell > existing.dwellFrames || newConfidence > existing.confidence) {
              return prev.map((t) => t.id === theftId ? { ...t, severity: (sevOrder[newSeverity] || 0) >= (sevOrder[t.severity] || 0) ? newSeverity : t.severity, dwellFrames: Math.max(t.dwellFrames, newDwell), confidence: Math.max(t.confidence, newConfidence) } : t);
            }
            return prev;
          }
          return [{ id: theftId, object_class: objClass, track_id: trackId, confidence: newConfidence, timestamp: parseTimestamp(data.timestamp), alert_reason: data.alert_reason, severity: newSeverity, dwellFrames: newDwell }, ...prev].slice(0, 5);
        });
        ...
        if (!alertedTrackIdsRef.current.has(trackId)) {
          ...
          setAlerts((prev) => prev.map((a) => {
            if (a.track_id === trackId && a.review_status === "unreviewed") {
              const sevOrder = { low: 0, medium: 1, high: 2 };
              if ((sevOrder[newSeverity] || 0) > (sevOrder[a.severity] || 0)) return { ...a, severity: newSeverity, confidence: Math.max(a.confidence, newConfidence) };
            }
            return a;
          }));
        }
        ...
        theftTimersRef.current[theftId] = setTimeout(() => setActiveThefts((prev) => prev.filter((t) => t.id !== theftId)), 30000);
      }

      setFps(data.fps ?? 0);
      setFpsHistory((prev) => [...prev.slice(1), { time: Date.now(), fps: data.fps ?? 0 }]);
      setDetectionHistory((prev) => [...prev.slice(1), { time: Date.now(), count: uniqueObjectCount }]);
  ...
  function handleTheftAccept(theftId) {
    ...
    setAlerts((prev) => prev.map((a) => {
      if (a.track_id === theft.track_id) {
        if (typeof a.id === "number" || (typeof a.id === "string" && !a.id.startsWith("live-"))) handleReview(a.id, "confirmed");
  ...
  function handleTheftReject(theftId) {
    ...
    setAlerts((prev) => prev.map((a) => {
      if (a.track_id === theft.track_id) {
        if (typeof a.id === "number" || (typeof a.id === "string" && !a.id.startsWith("live-"))) handleReview(a.id, "false-positive");
  ...
  function handleReview(alertId, status) {
    fetch(`${API_URL}/alerts/${alertId}/review`, {
      ...
      body: JSON.stringify({ outcome: status === "confirmed" ? "confirmed_theft" : status === "false-positive" ? "false_positive" : status, reviewed_by: "dashboard_user" }),
    }).catch((err) => console.error("Review failed:", err));
    setAlerts((prev) => prev.map((a) => (a.id === alertId ? { ...a, review_status: status } : a)));
  }

  const confirmedAlerts = alerts.filter((a) => a.review_status === "confirmed").length;
  const accuracy = alerts.length > 0 ? ((confirmedAlerts / alerts.length) * 100).toFixed(1) : "—";
    ...
    <div className="min-h-screen bg-surface-0 text-txt-primary font-sans">
      ...
      <header className="px-6 py-3 flex items-center justify-between border-b border-line">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm font-medium tracking-wide">Smart Checkout Detector</span>
          <span className="text-xs text-txt-muted ml-1">v1.0</span>
        </div>
        <div className="flex items-center gap-3">
          {hasActiveTheft && (
            <span className="text-xs bg-accent-red/15 text-accent-red px-3 py-1 rounded-full font-medium">
              {activeThefts.length} active alert{activeThefts.length > 1 ? "s" : ""}
            </span>
          )}
          <button onClick={resetSession} className="text-xs px-3 py-1.5 rounded bg-surface-2 text-txt-secondary hover:text-txt-primary transition-colors">
            New session
          </button>
          <button onClick={() => setShowSummary(true)} className="text-xs px-3 py-1.5 rounded bg-surface-2 text-txt-secondary hover:text-txt-primary transition-colors">
            End shift
          </button>
          <div className="flex items-center gap-1.5 ml-1">
            <div className={`w-1.5 h-1.5 rounded-full ${isConnected ? "bg-accent-green" : "bg-accent-red"}`} />
            <span className="text-xs text-txt-muted">{isConnected ? "Live" : "Offline"}</span>
          </div>
        </div>
      </header>
