...
def check_for_theft(
...
) -> tuple[bool, str | None, list, str | None, int]:
    """
    Core theft logic:
    ...
    Returns (is_alert, reason_string, scanned_items_list, severity, max_dwell)
    severity: "low" | "medium" | "high" based on dwell time and confidence
    """
    ...
    if suspicious_ids:
        ...
        # Calculate severity based on dwell time and confidence
        max_dwell = max(bag_zone_dwell.get(tid, 0) for tid in suspicious_ids)
        avg_conf = sum(
            obj["confidence"] for obj in tracked_objects
            if obj.get("track_id") in suspicious_ids
        ) / len(suspicious_ids)

        # Severity tiers:
        #   low    = just crossed threshold (3-8 frames) OR low confidence (<60%)
        #   medium = moderate dwell (9-20 frames) AND decent confidence (60-80%)
        #   high   = long dwell (20+ frames) OR high confidence (>80%)
        if max_dwell >= 20 or avg_conf >= 0.80:
            severity = "high"
        elif max_dwell >= 9 and avg_conf >= 0.60:
            severity = "medium"
        else:
            severity = "low"

        return True, reason, scanned_items, severity, max_dwell

    return False, None, scanned_items, None, 0
...
def push_to_cloud(payload: dict):
    """Fire-and-forget HTTP POST in a background thread so YOLO loop isn't blocked."""
    def _send():
        try:
            resp = requests.post(
                CLOUD_API_URL,
            ...
            print("[WARN] Cloud push timed out")
...
def main():
    ...
    while True:
        ...
        # ── Theft detection ──────────────────────────────────────
        is_alert, alert_reason, scanned_items, severity, dwell_count = check_for_theft(scan_ids, bag_ids, tracked_objects)
